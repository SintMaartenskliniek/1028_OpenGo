...
Authors: C.J. Ensink, B. Nienhuis (initial version)
"""
...
def readmarkerdata (filepath, **kwargs):
    ...
    # Itereer door elke key in de analogdata-dictionary
    for key, data in analogdata.items():
        afgehaalde_data[key] = data[10000:-5000]
        
        analogdata = afgehaalde_data
    


    afgehaalde_markerdata = {}

    # Itereer door elke key in de markerdata-dictionary
    for key, data in markerdata.items():
        afgehaalde_markerdata[key] = data[1000:-500, :]

        markerdata = afgehaalde_markerdata
